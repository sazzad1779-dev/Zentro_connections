...
export const ServiceDetailModal: React.FC<ServiceDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#071A36]/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-3xl bg-[#0B2854] text-white rounded-3xl shadow-2xl border border-white/15 overflow-hidden my-8">
        
        {/* Header */}
        <div className="p-8 bg-[#071A36] text-white relative border-b border-white/10">
          <button
            onClick={onClose}
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-xs font-mono font-bold text-[#00B8E6] mb-3 border border-white/10">
          ...
          <p className="text-xs sm:text-sm text-blue-100/90 mt-2 max-w-xl">
            {service.tagline}
          </p>
        </div>
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-blue-200/70 font-mono mb-2">Overview</h4>
          ...
          <div className="p-6 rounded-2xl bg-[#071A36] border border-white/10">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white flex items-center gap-2 mb-4 font-mono">
              <Layers className="w-4 h-4 text-[#00B8E6]" />
              <span>Full Scope & Key Deliverables</span>
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {service.deliverables.map((deliv, idx) => (
                <div key={idx} className="flex items-start gap-2 text-xs font-medium text-blue-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl bg-[#071A36] border border-white/10">
              ...
              <div className="flex flex-wrap gap-1.5">
                {service.technologies.map((t, idx) => (
                  <span key={idx} className="px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-[11px] font-mono text-blue-100">
                    {t}
                  </span>
                ))}
              </div>
            ...
            <div className="p-5 rounded-2xl bg-[#071A36] border border-white/10">
              <h5 className="text-xs font-bold text-white uppercase tracking-wider mb-1 flex items-center gap-1.5 font-mono">
                <TrendingUp className="w-4 h-4 text-[#00B8E6]" />
                <span>Expected Client ROI Impact</span>
              </h5>
              <div className="text-base sm:text-lg font-bold text-[#00B8E6] font-['Space_Grotesk'] mt-2">
